...
public class TransactionController : ControllerBase {
  ...
  /* Endpoint do konwersji kwoty z jednej waluty na drugą.
   Endpoint przyjmuje parametry: amount (kwota), from (waluta źródłowa), to (waluta docelowa).
  Wykorzystuje serwis CurrencyConverterService do pobrania kursów walut i przeliczenia kwoty.*/
  [HttpGet("convert")]
  [Authorize]
  public async Task<IActionResult> Convert(decimal amount, string from, string to) {
    var result = await _transactionService.ConvertAsync(amount, from, to);
    return Ok(result);
  }
}